...
function fbzlAjax(data,whichstep){
    ...
    if(whichstep == 'requestFbzllist'){
       ...
       //var url = '/apps/data/fbzl.json';
        ...
        if(parent&&parent.sxsl_store&&parent.sxsl_store.swsxDm==='60090102'){
            url='/hgzx-gld/api/hgzx/dbsx/fbzllist/tjdata/'+sqxh;
        }
    }
    ...
    $.ajax({
        ...
        success: function(res){
            if(res.success){
                ...
                if(!!parent.sxsl_store){//事项跟踪的补正资料不进来
                	 if(parent.sxsl_store.swsxDm != '' && parent.sxsl_store.swsxDm=='110132'){//外出报验需要手工添加一笔附报资料
                     	var fbzl={
                                 "swsxDm": "110132",
                                 "swsxMxDm": null,
                                 "fbzlDm": "000578",
...
function ylonRenderOpearte(e){
    ...
    if(record.scCount == '0'){
       // html = '<a href="javascript:uploadFbzl(\'' + index + '\');" class="upload">选择</a>';
    	...
    	var bysqxh=record.bysqxh;
    	var swsxDm=record.swsxDm;
    	if(bysqxh != undefined){
    		html =
    			'<a href="javascript:downloadPdf(\'' + bysqxh +'\',\''+swsxDm+
                '\');"  class="check-info">下载</a>';
    	}else{
    		html =
                '<a href="javascript:previewFbzl(\'' + index +
                '\');"  class="check-info">预览</a>';
...
//     return s;
// }
function downloadPdf(bysqxh,swsxdm){
	window.open('/dzgzpt-wsys/api/wcjyby/sqb/' + bysqxh+'/'+swsxdm);
}
