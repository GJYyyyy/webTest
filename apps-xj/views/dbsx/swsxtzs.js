...
function querySwsxtzsxx(lcslId, rwbh, blztDm, swsxDm, blxxData, otherData, rwztDm, djxh, sxslStoreObj) {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            $(".content_swsxMc3").text(result.value.swsxmc);
            $("#content_sqrq").text(mini.formatDate(mini.parseDate(result.value.sqrq), "yyyy年MM月dd日"));
            $(".content_sqrq2").text(mini.formatDate(mini.parseDate(result.value.sqrq), "yyyy年MM月dd日"));
            ...
            if (WSYS_BLZT_DM.BLZT_SLBTG == blztDm) {
                ...
                /**
                 * http://192.168.2.82:8080/redmine/issues/463937
                 * 增值税开票限额审批30090105  对纳税人变更纳税定额的核准30090104 企业印制发票审批30090101
                 */
                if (swsxDm === '30090105' || swsxDm === '30090104' || swsxDm === '30090101' || swsxDm === '60090105') {
                    $('#swxzxk_normal,.content_syyj,#swsxtzsTy,#swsxtzsNr').css('display', 'none')
                    $('#content_nsrmc').css('margin-left', '20px')
                    $('.swsxtzs-xzxk-bysl').css('display', 'block')
                }
            }
            ...
            if (WSYS_BLZT_DM.BLZT_BZZL == blztDm) {
                 ...
                 * 增值税开票限额审批30090105
                 */
                if (swsxDm === '30090105' || swsxDm === '60090105') {
                    $('#swxzxk_normal,.content_syyj,#content_blzt_sm,#swsxtzsTy').css('display', 'none')
                    $('#content_nsrmc').css('margin-left', '20px')
                    $('.swsxtzs-xzxk-bzzl').css('display', 'block')
                }
            }
...
function dealTsgwAfter(tsjgblxx, messageid) {
    ...
    var xzspSwsxdm = ['30090101', '30090102', '60090102', '30090103', '30090104', '30090105', '30090106', '30090107', '30010415', '60090105'];
    ...
    if (xzspSwsxdm.indexOf(SWSX_STORE.swsxdm) >= 0) {
        url = ['60090105', '60090102'].indexOf(SWSX_STORE.swsxdm) > -1 ? '/hgzx-gld/api/hgzx/dbsx/saveSwsxBl' : '/hgzx-gld/api/hgzx/dbsx/saveSwsxBl';
            ...
            success: function (data) {
                ...
                if (resultData.success) {
                    if (WSYS_BLZT_DM.BLZT_DSP == mini.get("blztDm").getValue()) {
                        ...
                        } else if (SWSX_STORE.swsxdm == '11041502' && WSYS_BLZT_DM.BLZT_SLTG == mini.get("blztDm").getValue()) {
                            $.ajax({
                                url: "/dzgzpt-wsys/api/ljtssdba/pushRwdb",
                                type: "post",
                                contentType: 'application/json; charset=UTF-8',
                                data: mini.encode({
                                    sqxh: mini.get("sqxh").getValue() || ''
                                ...
                                success: function (data) {
                                    var result = mini.decode(data);
                                    if (!result.success) {
                                        mini.alert(result.message || '生产任务中心待办失败,请重试。', '提示信息', function () {
                                            onCancel('ok');
                                        });
                                    } else {
                                        mini.alert("已生成任务中心待办，请至岗位待办查看。", '提示信息', function () {
                                            onCancel('ok');
                                        });
                                    }
                                }
...
function saveSwsxBj() {
    var blztDm = mini.get("blztDm").getValue()
    if (SWSX_STORE.swsxdm == '110407' && blztDm === '01') { // 增值税即征即退备案
        return saveSwsxBjMethods()
    }
    if (SWSX_STORE.swsxdm == '110110' && blztDm === '01') { //分支机构信息确认前置调金三小程序
        var messageId = mini.loading("调用金三小程序中, 请稍等 ...", "提示");

        setTimeout(function () {
            $.ajax({
                url: "/dzgzpt-wsys/api/sh/fzjgdlns/fp/zgswjg",
                type: "POST",
                contentType: 'application/json; charset=UTF-8',
                async: false,
                data: mini.encode({
                    sqxh: mini.get("sqxh").getValue() || '',
                    djxh: SWSX_STORE.djxh || ''
                }),
                success: function (data) {
                    if (data.success) {
                        mini.hideMessageBox(messageId);
                        mini.alert("调用成功！", "提示", function (e) {
                            saveSwsxBjMethods()
                        });
                    } else {
                        mini.hideMessageBox(messageId);
                        mini.alert(data.message || "请求错误，请稍候再试！")
                    }
                },
                error: function (data) {
                    mini.hideMessageBox(messageId);
...
function saveSwsxBjMethods() {
    ...
    var tsjgblxx = "";
    if (('110107' == SWSX_STORE.swsxdm || '30090105' == SWSX_STORE.swsxdm || '60090105' == SWSX_STORE.swsxdm || '30090102' == SWSX_STORE.swsxdm || '60090102' == SWSX_STORE.swsxdm || SWSX_STORE.swsxdm == '30090106') && WSYS_BLZT_DM.BLZT_DSP == mini.get("blztDm").getValue()) {
        mini.hideMessageBox(messageid);
        ...
        if (smrzflag) {
            ...
            submitData = mini.encode(submitData);
            var xzspSwsxdm = ['30090101', '30090102', '60090102', '30090103', '30090104', '30090105', '60090105', '30090106', '30090107', '30010415'];
            var contentType = 'application/x-www-form-urlencoded;charset=UTF-8';
            var saveSwsxBlPatam = {
                data: submitData,
                lcslId: mini.get("lcslId").getValue(),
            ...
            if (xzspSwsxdm.indexOf(SWSX_STORE.swsxdm) >= 0) {
                // 创新班 增加税务事项，切换url
                url = '/hgzx-gld/api/hgzx/dbsx/saveSwsxBl';
                if (SWSX_STORE.swsxdm.charAt(0) === '6') {
                    url = '/hgzx-gld/api/hgzx/dbsx/saveSwsxBl';
                }
                contentType = 'application/json;charset=UTF-8';
                saveSwsxBlPatam.swsxtzsDto = {
            ...
            var blztDm = mini.get("blztDm").getValue()
            if (SWSX_STORE.swsxdm === '110407' && blztDm === '01') {
                url = '/dzgzpt-wsys/api/wtgl/dbsx/saveSwsxBl'
                saveSwsxBlPatam.lcslId = SWSX_STORE.sxslStoreObj.lcslId
                saveSwsxBlPatam.blztDm = '01'
            }
            setTimeout(function () {
                $.ajax({
                    url: url,
                    ...
                    success: function (data) {
                        ...
                        if (resultData.success) {
                            if (WSYS_BLZT_DM.BLZT_DSP == mini.get("blztDm").getValue()) {
                                ...
                                } else if (SWSX_STORE.swsxdm == '110418' && WSYS_BLZT_DM.BLZT_SLTG == mini.get("blztDm").getValue()) {
                                    mini.alert("受理成功。请在电子工作平台操作“金三结果通知书制发”。", '提示信息', function () {
                                        onCancel('ok');
                                    });
                                } else {
                                    mini.alert("受理成功！", '提示信息', function () {
...
function selectTsSwjg(sqxh, swsxDm, flag, messageid) {
    var messageid = mini.loading("查询中, 请稍等 ...", "查询中");
    // 创新班 税务事项60090105，切换url
    var url = swsxDm == '60090105' ? '/hgzx-gld/api/hgzx/dbsx/queryReceiveJgDmAndGwXhList/' : '/hgzx-gld/api/hgzx/dbsx/queryReceiveJgDmAndGwXhList/'
    $.ajax({
        url: url + sqxh + '/' + swsxDm,
        type: "post",
        data: "",
