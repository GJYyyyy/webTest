...
/*		if(sqzl.blzt.swsxDm === '110401'){
    ...
    /**
     * 增值税专用发票（增值税税控系统）最高开票限额审批受理
     */
    if (sqzl.blzt.swsxDm === '30090105'||sqzl.blzt.swsxDm === '60090105') {
        var viewData = sqzl.blzt.sqsxData.viewData
        viewData = JSON.parse(viewData)
        var cwfzrglqtfzcqyhs = viewData['swxzxk-yltj'].cwfzrglqtfzcqyhs
        var fddbrglqtfzcqyhs = viewData['swxzxk-yltj'].fddbrglqtfzcqyhs
        if (cwfzrglqtfzcqyhs && fddbrglqtfzcqyhs && (fddbrglqtfzcqyhs !== '0' || cwfzrglqtfzcqyhs != '0')) {
            $('#tabs1').after('<h3 style="color:red;padding-left:20px">该纳税人存在以下风险：其法人或财务负责人曾任非正常户或走逃失联企业的法人或财务负责人的纳税人。</h3>')
        }
    }
    /**
     * http://gd-zentao.dc.servyou-it.com/index.php?m=task&f=view&taskID=12798
     * 【对纳税人延期申报的核准】、【增值税最高开票限额审批】、【对采取实际利润额预缴以外的其他企业所得税预缴方式的核定】
     *  隐藏【不予受理（撤回）】按钮
     */
    if (sqzl.blzt.swsxDm === '30090102' || sqzl.blzt.swsxDm === '30090105' || sqzl.blzt.swsxDm === '30090106'||sqzl.blzt.swsxDm === '60090105') {
        $('#byslBtn').hide()
    }
};
