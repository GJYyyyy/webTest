...
var dcrydcxc = {
    checkysyjData: [{ id: 'Y', text: '认定非正常户' }, { id: 'N', text: '不认定非正常户'}],
    GridDatas:{
        data:[],
        total:0
    },
    ...
    openSwryCheck: function(record){
        ...
        record.ysyj && mini.get("ysyj").select(Number(record.ysyj === "Y" ? 1 : 2) - 1);
        record.ysyjsm && mini.get("ysyjsm").setValue(record.ysyjsm);
    ...
    checkSwryOk: function(){
        ...
        var ysyjmc = mini.get('ysyj').text;
        var ysyjsm = mini.get('ysyjsm').getValue();
        if(ysyjsm && ysyjsm.length > 500){
        ...
        if(!dcrydcxc.plfp){
            ...
            dcrydcxcGridData[rowIndex].ysyj = formdata.ysyj;
            dcrydcxcGridData[rowIndex].ysyjmc = ysyjmc;
            dcrydcxcGridData[rowIndex].ysyjsm = formdata.ysyjsm;
            dcrydcxc.GridDatas.data[rowIndex+pageIndex*pageSize] = dcrydcxcGridData[rowIndex];
        } else{
            var rows = dcrydcxc.selectMaps;
            for(var i = 0;i < rows.length;i++){
                for(var j=0;j<rows[i].length;j++){
                    dcrydcxc.GridDatas.data[rows[i][j]['_number']].ysyj = formdata.ysyj;
                    dcrydcxc.GridDatas.data[rows[i][j]['_number']].ysyjmc = ysyjmc;
                    dcrydcxc.GridDatas.data[rows[i][j]['_number']].ysyjsm = formdata.ysyjsm;
                }
            }
        }
    ...
    queryGrid: function () {
        $.ajax({
            url: '/dzgzpt-wsys/api/sh/fzchrd/query/ysdata?processInstanceId=' + getQueryString("processInstanceId"),
            type: 'get',
            async: false,
...
function ysyjmcRenderer(e){
    var record = e.record;
    var rowIndex = e.rowIndex;
    var ysyjmc = record.ysyjmc;
    return '<a class="color-blue wh100 inlineblock lineH36" onclick="dcrydcxc.openSwryCheck(record)">' + (ysyjmc?(ysyjmc):'请选择') + '</a>';
}
function ysyjsmRenderer(e){
    var record = e.record;
    var rowIndex = e.rowIndex;
    var ysyjsm = record.ysyjsm;
    return '<a class="color-blue wh100 inlineblock lineH36" onclick="dcrydcxc.openSwryCheck(record)">' + (ysyjsm?(ysyjsm):'请选择') + '</a>';
}
...
function saveData(e){
    ...
    $.ajax({
        url: '/dzgzpt-wsys/api/sh/fzchrd/save/ysdata',
        type: 'POST',
        async: false,
        contentType:'application/json; charset=UTF-8',
