 ...
 */
var sxslcommon = {
    lcslId: getUrlParamByName('lcslId'),
    isDw: getUrlParamByName('isDw'),
    sqxh: getUrlParamByName('sqxh'),
    swsxDm: getUrlParamByName('swsxDm'),
    blztDm: getUrlParamByName('blztDm')
};
 ...
 * 请求文书申请基本信息
 */
sxslcommon.swsxsqJbxx = sxslService.queryWssqJbxx(sxslcommon.lcslId);
...
sxslcommon.initSxslPage = function (data) {
    ...
    if (!!data) {
        ...
        // 将数据存到对应的对象中
        sxsl_store.lcslId=sxslcommon.lcslId;
        // sxsl_store.rwbh=sxslcommon.lcslId.rwbh;
        // 补录资料URL地址，为空说明不需要补录：
        sxsl_store.sqxh = sxslcommon.sqxh;
        sxsl_store.swsxDm = sxslcommon.swsxDm;
        sxsl_store.nsrsbh = sxslcommon.swsxsqJbxx.sqr;
        sxsl_store.sxshfsDm = sxslcommon.swsxsqJbxx.sxshfsDm;
        if('110207' == sxsl_store.swsxDm){
        	sxsl_store.sxshfsDm = WSYS_SLSHFS_DM.SLSHFS_FJB;
        }
        //sxsl_store.nsrmc=data.nsrmc;
        fbzlSqxh = sxslcommon.sqxh;
        sxsl_store.rwztDm = sxslcommon.swsxsqJbxx.rwztDm;
        sxsl_store.djxh = "";
        // 加载信息特色信息
        ...
        mini.parse($('#tabs1'));
        if (data.swsxDm == "110133" || data.swsxDm == "110212" || data.swsxDm == "110805") {//外经证申报则不显示第二个tab页
            tabs.removeTab(1);
        }*/
        ...
        //初始化 文书申请信息
        sxsl_store.sqsxData = sxslService.queryWssqxxData(sxslcommon.sqxh);
        sxsl_store.sqsxData = mini.decode(sxsl_store.sqsxData);
        //287457显示申请人及手机号
        $('#sjhm').text('（' + sxsl_store.sqsxData.sjhm + ')');
        ...
        if (!sxsl_store.sqsxData.viewData) {
            ...
            sqzl.initPage(sxsl_store.ylUrl, mini.decode(sxsl_store.sqsxData.viewData));
        }
    }
    return flag;
};
...
function openSxbjSwsxtzs(blxxData, blztFlag, storeobj, otherData) {
    ...
    mini.open({
        ...
        onload: function () {
            var iframe = this.getIFrameEl();
            iframe.contentWindow.querySwsxtzsxx(
                sxslcommon.sqxh,
                rwbh,
                blztFlag,
                swsxDm,
...
function displayButton(swsxDm, sxshfsDm) {
    ...
    mini.get("sqbPrintBtn").hide();
        ...
        if (WSYS_SLSHFS_DM.SLSHFS_JB == sxsl_store.sxshfsDm) {
            ...
            /*if (!(swsxDm == "110133" || swsxDm == "110212"|| swsxDm == "110001" || swsxDm == "110121")) {//外经证申报不显示补正资料
