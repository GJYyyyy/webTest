...
//修复在ie8下无法旋转的问题
    function fnRotateScale(angle, scale ,obj) {
        var angle = parseFloat(angle) || 0;
        var scale = parseFloat(scale) || 1;
        var rad = angle * (Math.PI / 180);
        var m11 = Math.cos(rad) * scale, m12 = -1 * Math.sin(rad) * scale, m21 = Math.sin(rad) * scale, m22 = m11;
        var width=parseFloat(obj.width),height=parseFloat(obj.height);
        var dx=-width/2*Math.cos(rad)+height/2*Math.sin(rad)+width/2,dy=-width/2*Math.sin(rad)-height/2*Math.cos(rad)+height/2;
        var filter = "progid:DXImageTransform.Microsoft.Matrix(Dx="+dx+",Dy="+dy+",M11="+ m11 +",M12="+ m12 +",M21="+ m21 +",M22="+ m22 +",SizingMethod='auto expand')";
        return filter;
    };
    ...
    function getFilter(options){
        ...
        if (document.querySelector && !window.addEventListener){
            ...
            if (isNumber(angle)) {
                return fnRotateScale(angle,scaleX,options);
            }
        }else{
            return "";
        }
        ...
        click: function (e) {
            ...
            switch (action) {
                ...
                case 'rotate-left':
                    this.rotate(-90);
                    flag=(++flag)%2;
                    if(document.querySelector && !window.addEventListener&&flag==1){
                        this.$image.css({
                            position:'relative',
                            left:(this.$image.height()-this.$image.width())/2+'px',
                            top:(this.$image.width()-this.$image.height())/2+'px'
                        })
                    };

                    if(document.querySelector && !window.addEventListener&&flag==0){
                        this.$image.css({
                            position:'relative',
                            left:0,
                            top:0
    ...
    Viewer.TEMPLATE = (
        ...
        '<ul class="viewer-toolbar" style="width:184px">' +
        '<li class="viewer-zoom-in" data-action="zoom-in" title="放大"></li>' +
        '<li class="viewer-zoom-out" data-action="zoom-out" title="缩小"></li>' +
        '<li class="viewer-one-to-one" data-action="one-to-one" style="display:none"></li>' +
        '<li class="viewer-prev" data-action="prev" title="上一张"></li>' +
        '<li class="viewer-play" data-action="play" title="全屏"></li>' +
        '<li class="viewer-next" data-action="next" title="下一张"></li>' +
        '<li class="viewer-rotate-left" data-action="rotate-left" style="display:none"></li>' +
        '<li class="viewer-rotate-right" data-action="rotate-right" title="旋转"></li>' +
        '<li class="viewer-reset" data-action="reset" title="重置"></li>' +
        '<li class="viewer-flip-horizontal" data-action="flip-horizontal" style="display:none"></li>' +
        '<li class="viewer-flip-vertical" data-action="flip-vertical" style="display:none"></li>' +
        '</ul>' +
